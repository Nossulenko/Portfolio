...
class Component extends React.PureComponent {
  ...
  getDuration () {
    ...
    // the actual text.
    const realDuration = (1000 / 60) * String(children).length;
  ...
  animateTransition (isEntering) {
    ...
    this.setOverlayText(isEntering ? '' : String(children));

    const onCall = ({ timeProgress }) => {
      const newLength = Math.round((timeProgress * String(children).length) / duration);

      const newText = String(children).substring(0, newLength);
      this.setOverlayText(newText);

      this.playSound();

      return isEntering ? newLength <= String(children).length : newLength > 0;
    };
  ...
  animateTransform (isEntering) {
    ...
    const initialText = getRandomCharacters(String(children).length, randomCharacters);
    this.setOverlayText(initialText);

    const onCall = ({ timeProgress }) => {
      const newLength = Math.round((timeProgress * String(children).length) / duration);

      const newText1 = String(children).substring(0, newLength);
      const newText2 = getRandomCharacters(String(children).length - newLength, randomCharacters);
